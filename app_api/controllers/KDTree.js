let AXIS_X  = 0;
	let AXIS_Y = 1;
	let K = 2; // I only require a 2d k-d tree & currently I'm too lazy to calculate bounding hyperrectangles, 2d ones will do just fine.
	 ...
	 * @returns The nearest neighbour point {x:?, y:?} to the searchCoord that was found within the tree.
	 */
	KDTree.prototype.getNearestNeighbours = function(searchCoord, maxResults, opt_consideredPoints, radius) {
		let results = [];
		...
		for (let i = 0; i < results.length; ++i) {
			let distance = getSquaredEuclidianDistance(searchCoord.x, searchCoord.y, results[i].node.point.x, results[i].node.point.y);
	...
	KDTree.prototype.getNearestNeighbours_ = function(currNode, searchCoord, depth, results, maxResults, opt_consideredPoints) {
		...
		let axis = depth % K;
		let currNodeDistanceToDesiredCoord = getSquaredEuclidianDistance(searchCoord.x, searchCoord.y, currNode.point.x, currNode.point.y);
		let bestSeen = {node:currNode, distance: currNodeDistanceToDesiredCoord};
		insertResult_(results, bestSeen, maxResults );
		let searchNodeSplittingCoord = axis == AXIS_X ? searchCoord.x 	 : searchCoord.y;
		let currNodeSplittingCoord   = axis == AXIS_X ? currNode.point.x : currNode.point.y;
		
		let searchLeft    = searchNodeSplittingCoord < currNodeSplittingCoord;
		let targetChild   = searchLeft ? currNode.leftChild  : currNode.rightChild;
		let oppositeChild = searchLeft ? currNode.rightChild : currNode.leftChild;
		...
		if (oppositeChild) {
			...
			let squaredDist = getSquaredEuclidianDistance(searchCoord.x, searchCoord.y, toX, toY);
	...
	function insertResult_(results, insertNode, maxResults) {
		...
		for (insertIndex = results.length - 1; insertIndex >= 0; --insertIndex) {
			let nearestNeighbourNode = results[insertIndex];
